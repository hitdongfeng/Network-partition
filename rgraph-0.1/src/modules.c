/*
  modules.c
  $LastChangedDate: 2007-11-28 18:33:02 -0600 (Wed, 28 Nov 2007) $
  $Revision: 109 $
*/
...
/*
  ---------------------------------------------------------------------
  Print the nodes and the group to which they belong! 
  ---------------------------------------------------------------------
*/
void
FPrintNodesInfo(FILE *outf, struct group *partition, double timet, struct node_gra *net)
{
  struct group *g = partition;
  struct node_lis *p;
  //fprintf(outf, "i %f %d %f %d\n", timet, CountNodes(net), Modularity(g,timet), NumberOfModules(g));
...
*/
double
Modularity(struct group *part, double time)
{
  struct group *g = part;
  int links2 = 0;
  double modul = 0.0;
  ...
  g = part;
  while ((g = g->next) != NULL)
    modul += time * (double)(2 * g->inlinks) / (double)links2 -
...
int
NumberOfModules(struct group *part)
{
  struct group *g = part;
  int nModules = 0;
...
*/
double
ModularityWeight(struct group *part, double time)
{
  struct group *g = part;
  double links2 = 0.0;
  double modul = 0.0;
  ...
  g = part;
  while (( g = g->next) != NULL)
    modul += time * (double)(2 * g->inlinksW) / (double)links2 -
	...
	for (i=0; i<nnod; i++) {
	  ...
	  dE += time * (double)(2*glist[oldg]->inlinks - 2*inold) /
	    ...
	    (double)(glist[oldg]->totlinks + glist[oldg]->inlinks -
		     nlink ) /
	    ((double)totallinks * (double)totallinks);
	  dE += time * (double)(2*glist[newg]->inlinks + 2*innew) /
	    (double)totallinks -
	    (double)(glist[newg]->totlinks + glist[newg]->inlinks +
		     nlink ) *
	    (double)(glist[newg]->totlinks + glist[newg]->inlinks +
  ...
  while (T > Tf && count < limit) {
    ...
    case 'v':
      fprintf(stderr, "%g %lf %lf %g\n",
	      1.0/T, energy, Modularity(part, time), T);
      break;
    case 'd':
	...
	if (glist[g1]->size < nnod) { /*  Unless all nodes are together */
	  ...
	  dE = 0.0;
	  dE -= time * (double)(2*glist[g1]->inlinks) / (double)totallinks -
	    ((double)(glist[g1]->totlinks + glist[g1]->inlinks) *
	     (double)(glist[g1]->totlinks + glist[g1]->inlinks) ) /
	    ((double)totallinks * (double)totallinks );
	  dE -= time * (double)(2*glist[g2]->inlinks) / (double)totallinks -
	    ((double)(glist[g2]->totlinks + glist[g2]->inlinks) *
	     (double)(glist[g2]->totlinks + glist[g2]->inlinks) ) /
	    ((double)totallinks * (double)totallinks );
	  dE += time * 2.0*(double)(glist[g1]->inlinks +
	...
	if (empty >= 0) { /*  if there are no empty groups, do nothing */
	  ...
	  split = SAGroupSplit(glist[dice], Ti, T, 0.95, 1, gen, time);
	  ...
	  dE = 0.0;
	  dE -= time * (double)(2*glist[dice]->inlinks) /
	    (double)totallinks -
	    ((double)(glist[dice]->totlinks+glist[dice]->inlinks) *
	     (double)(glist[dice]->totlinks+glist[dice]->inlinks))/
	    ((double)totallinks * (double)totallinks );
	  dE -= time * (double)(2*glist[empty]->inlinks) /
	    (double)totallinks -
	    ((double)(glist[empty]->totlinks + glist[empty]->inlinks) *
	     (double)(glist[empty]->totlinks + glist[empty]->inlinks))/
	    ((double)totallinks * (double)totallinks );
	  dE += time * 2.0*(double)(glist[dice]->inlinks+
	...
	//RENAUD!!!
	//if(energy!=energyant)
	//fprintf(stderr, "# %f %f %f\n", Modularity(part, time), energy, energyant);  
